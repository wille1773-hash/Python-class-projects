...
- `pakudex_cli/` – Menu-driven creature tracker (add/list/show/evolve/sort).
- `calc_utils/` – Calculator functions with pytest coverage.
- `whack_a_mole/` – Pygame whack-a-mole grid game.
- `cow_talk/` – Cowsay-style ASCII animals with variants.
- `extra_credit_tickets/` – Ticket pricing flow with validation.

How to use:
1) `python -m venv .venv && source .venv/bin/activate` (or use your Python env).
2) For Pygame projects: `pip install pygame`.
3) See each subfolder README for commands, inputs, and notes.
